...
export class ColumnConfigComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.columns = this.getAviables(
      this.config.data.columns.slice(),
      this.config.data.selected.slice()
    );
  ...
  save() {
    this.ref.close(
      this.getSelected(this.config.data.columns.slice(), this.selectedColumns)
    );
  }
